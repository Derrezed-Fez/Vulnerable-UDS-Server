...
long gm_lastcms = 0;
int seeds[10][5] = {{0xf0, 0xd7, 0xff, 0x2e, 0xff},{0xf0, 0xd8, 0xff, 0x2f, 0xff},{0xf0, 0xd9, 0xff, 0x30, 0xff}, {0xf0, 0xda, 0xff, 0x31, 0xff}, {0xf0, 0xdb, 0xff, 0x32, 0xff}, {0xf0, 0xdc, 0xff, 0x33, 0xff}, {0xf0, 0xdd, 0xff, 0x34, 0xff}, {0xf0, 0xde, 0xff, 0x35, 0xff}, {0xf0, 0xdf, 0xff, 0x36, 0xff}, {0xf0, 0xe0, 0xff, 0x37, 0xff}};
int seed_counter = 0;
bool seed_requested = false;
bool abs_unlocked = false;
int session_level = 0x00;
...
Brake Controller
*/
void handle_abs_780(int can, struct canfd_frame frame) {
  if(verbose) plog("Received ABS 0x780 request\n");
  char resp[150];
  if(frame.data[0] == 0x30) {
    flow_control_push_to(can, 0x786);
    return;
  }
  switch(frame.data[1]) {
    case 0x10:
    ...
    case 0x22: // Read Data By Identifier
      if(frame.data[2] == 0xF1) {
        switch(frame.data[3]) {
        case 0x90: // VIN
          if(verbose) plog("Read data by ID 0xF190\n");
          resp[0] = frame.data[1] + 0x40;
          resp[1] = frame.data[2];
          resp[2] = 0x90;
          resp[3] = 0x31;
          ...
          resp[18] = 0x37;
          resp[19] = 0x36;
          isotp_send_to(can, resp, 20, 0x786);
          break;
        case 0x91: // Vehicle Manufacturer ECU Hardware Number Data Identifier
          if(verbose) plog("Read data by ID 0xF191\n");
          resp[0] = frame.data[1] + 0x40;
          resp[1] = frame.data[2];
          resp[2] = 0x91;
          resp[3] = 0x46;
          ...
          resp[13] = 0x43;
          resp[14] = 0x55;
          isotp_send_to(can, resp, 15, 0x786);
          break;
        default:
          frame.can_id = 0x786;
          frame.len = 8;
      ...
      case 0x27:
        if(frame.data[2] == 0x03){
          if(verbose) plog("Request seed at level 03\n");
          if(session_level == 0x03) {
            frame.can_id = 0x786;
            frame.len = 8;
            frame.data[0] = 0x07;
            frame.data[1] = (int)frame.data[1] + 0x40;
            frame.data[2] = (int)frame.data[2];
            frame.data[3] = seeds[seed_counter][0];
            frame.data[4] = seeds[seed_counter][1];
            frame.data[5] = seeds[seed_counter][2];
            frame.data[6] = seeds[seed_counter][3];
            frame.data[7] = seeds[seed_counter][4];
            write(can, &frame, CAN_MTU);
            seed_requested = true;
            if(seed_counter == 10) {
              seed_counter = 0;
          ...
          if(seed_requested) {
            ...
            int counter = 0;
            for(int i = 4; i >= 0; i--){
              reversed_seed[counter] = seeds[seed_counter-1][i];
              counter++;
            }
            for(int i = 0; i < 5; i++) {
              if(frame.data[i+3] != reversed_seed[i]) {
                  if(verbose) plog("Invalid Key\n");
                  frame.can_id = 0x786;
                  frame.len = 8;
                  frame.data[0] = 0x03;
                  frame.data[1] = 0x7F;
            ...
            resp[25] = 0x64;
            resp[26] = 0x7D;
            isotp_send_to(can, resp, 27, 0x786);
          } else {
            if(verbose) plog("Security Unlock Attempt Without Seed Request\n");
            frame.can_id = 0x786;
            frame.len = 8;
            frame.data[0] = 0x03;
            frame.data[1] = 0x7F;
          ...
          if(verbose) plog("Security Unlock Attempt Invalid Access Level\n");
          frame.can_id = 0x786;
          frame.len = 8;
          frame.data[0] = 0x03;
          frame.data[1] = 0x7F;
    ...
    case 0x22: // Read Data By Identifier
      if(frame.data[2] == 0xF1) {
        ...
        case 0x90: // VIN
          ...
          resp[18] = 0x37;
          resp[19] = 0x36;
          isotp_send_to(can, resp, 20, 0x77A);
...
//
	...
	if(cfg_parse(cfg, "vulnerability.conf") == CFG_PARSE_ERROR)
		return 1;

	plog("ECU_RESET, %s!\n", cfg_getstr(cfg, "ECU_RESET"));
	plog("UNDOC_DID, %s!\n", cfg_getstr(cfg, "UNDOC_DID"));
	plog("P2_MAX, %s!\n", cfg_getstr(cfg, "P2_MAX"));
  ...
  //Freeing the configuration object
  cfg_free(cfg);
